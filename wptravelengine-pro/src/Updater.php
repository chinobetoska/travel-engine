...
class Updater {
	...
	const STORE_URL = 'https://wptravelengine.com';
	protected string $api_url = '';
	protected ?array $api_data = array();
	protected string $name = '';
	protected string $slug = '';
	protected $version = '';
	protected bool $wp_override = false;
	...
	/**
	 * Set up WordPress filters to hook into an update process.
	 *
	 * @return void
	 * @uses add_filter()
	 *
	 ...
	 * @uses api_request()
	 */
	public function check_update( object $_transient_data ) {
	 ...
	 * @param array $plugin
	 */
	public function show_update_notification( string $file, array $plugin ) {
		...
		if ( ! empty( $update_cache->response[ $this->name ] ) && version_compare( $this->version, $version_info->new_version, '<' ) ) {
			...
			if ( empty( $version_info->download_link ) ) {
				printf(
					esc_html__( 'There is a new version of %1$s available. %2$sView version %3$s details%4$s.', 'wptravelengine-pro' ),
				...
				printf(
					esc_html__( 'There is a new version of %1$s available. %2$sView version %3$s details%4$s or %5$supdate now%6$s.', 'wptravelengine-pro' ),
					esc_html( $version_info->name ),
					'<a target="_blank" class="thickbox" href="' . esc_url( $changelog_link ) . '">',
					esc_html( $version_info->new_version ),
					'</a>',
	 ...
	 *
	 */
	public function plugins_api_filter( $_data, string $_action = '', ?object $_args = null ) {
		...
		// Get the transient where we've stored the api request for this plugin for 24 hours
	 ...
	 * @since 3.6.5
	 */
	protected function convert_object_to_array( object $data ): array {
		$new_data = array();
	...
	/**
	 * Calls the API and, if the request is successful, returns the object delivered by the API.
	 *
	...
	public function show_changelog() {
		...
		if ( ! current_user_can( 'update_plugins' ) ) {
			wp_die( esc_html__( 'You do not have permission to install plugin updates', 'wptravelengine-pro' ), esc_html__( 'Error', 'wptravelengine-pro' ), array( 'response' => 403 ) );
		}
	...
	public static function from_extension( Extension $extension ): Updater {
		return new static(
			self::STORE_URL,
			$extension->file_path(),
			array(
				'version' => $extension->version,
				'license' => $extension->license_key(),
