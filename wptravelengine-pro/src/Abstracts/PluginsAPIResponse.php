...
namespace WPTravelEnginePro\Abstracts;
...
abstract class PluginsAPIResponse {
	...
	protected function prepare_plugins_api( object $data ): object {
		$data->sections     = $this->maybe_object_to_array( $data->sections ?? [] );
		$data->banners      = $this->maybe_object_to_array( $data->banners ?? [] );
		$data->icons        = $this->maybe_object_to_array( $data->icons ?? [] );
		$data->contributors = $this->maybe_object_to_array( $data->contributors ?? [] );
